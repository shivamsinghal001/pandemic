...
from ..data import ExperimentResult, H5DataLoader
from ..environment import PandemicSimConfig, sorted_infection_summary
from ..viz import (plot_critical_summary, plot_global_infection_summary,
...
def make_evaluation_plots_from_data(
    ...
    annotate_stages = (
        [annotate_stages] * n_params
        if isinstance(annotate_stages, bool)
        else annotate_stages
    )
    ...
    if show_summary_plots:
        figsize = (
            figsize if figsize is not None else ((16, 6) if n_params <= 5 else (20, 12))
        )
        ...
        for i, (exp_result, param_label, ann_stages) in enumerate(
            zip(data, param_labels, annotate_stages)
        ):
            plot_global_infection_summary(
                exp_result, ax=axs[i, 0], annotate_stages=ann_stages
            )
            if show_stage_trials:
                seed_indices = np.random.permutation(
                    len(exp_result.obs_trajectories.stage.shape[1])
                )[:3]
            ...
            for j, ax in enumerate(axs[i]):
                ...
                axs[i, j].annotate(
                    f"({plot_ref_labels[plot_ref_label_i]})",
                    (0.5, 0.0),
        ...
        axs[0, 0].legend(
            handles,
            gis_legend + ["Max hospital capacity", "cumulative_reward"],
            fancybox=True,
            loc="best",
        ...
        bar_plots_2d = (
            3
            + int(show_time_to_peak)
            + int(show_pandemic_duration)
