...
def seeded_experiment_main(
    ...
    stage_dict = {
        f"stage_{i}": (s.stage, s.end_day if s.end_day is not None else -1)
...
def experiment_main(
    ...
    while num_evaluated_seeds < num_random_seeds:
        ...
        print(
            f"Running experiment seed: {seed} - {num_evaluated_seeds + 1}/{num_random_seeds}"
        )
        ret = seeded_experiment_main(
            exp_id=exp_id,
            sim_config=sim_config,
            sim_opts=sim_opts,
