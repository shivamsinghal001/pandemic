...
class CityRegistry(Registry):
    ...
    def register_person(self, person: Person) -> None:
        ...
        if person.state.current_location not in self._location_register:
            raise RegistrationError(
                f"Unable to register the person {person.id.name} because the current"
        ...
        for loc_id in person.assigned_locations:
            if loc_id not in self._location_register:
                raise RegistrationError(
                    f"Unable to register the person {person.id.name} because the assigned"
                    f"location {loc_id.name} is not registered yet."
                )

        current_location = self._location_register[person.state.current_location]
        assigned_locations = [
            self._location_register[loc_id] for loc_id in person.assigned_locations
        ]

        if (
            current_location.id not in person.assigned_locations
            and person.id not in current_location.state.persons_in_location
        ):
            if not current_location.is_entry_allowed(person.id):
                raise RegistrationError(
                    "Unable to register the person because the person is not allowed in his/her"
    ...
    def register_person_entry_in_location(
        ...
        if (
            next_location.id != current_location.id
        ...
        if type(next_location) not in self.IGNORE_LOCS_SUMMARY:
            ...
            self._global_location_summary[key] = dataclasses.replace(
                ...
                visitor_count=(person_cnt * summary.visitor_count + is_visitor)
                / person_cnt,
            )
        return True

    def update_location_specific_information(self) -> None:
        self._location_ids_with_social_events = [
            loc_id
            for loc_id, loc in self._location_register.items()
    ...
    def location_ids_of_type(
        ...
        return tuple(
            loc_id
            for loc_id, loc in self._location_register.items()
            if isinstance(loc, location_type)
        )

    def get_persons_in_location(self, location_id: LocationID) -> Set[PersonID]:
        return cast(
            LocationState, self._location_register[location_id].state
        ).persons_in_location
    ...
    def get_location_work_time(self, location_id: LocationID) -> SimTimeTuple:
        assert (
            location_id in self._business_location_ids
        ), "The given location id is not a business location."
        return cast(
            BusinessBaseLocation, self._location_register[location_id]
        ).get_worker_work_time()
