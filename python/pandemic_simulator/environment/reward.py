...
__all__ = [
    ...
    "SumReward",
    "UnlockedBusinessLocationsReward",
    "InfectionSummaryIncreaseReward",
    "InfectionSummaryAboveThresholdReward",
    "LowerStageReward",
    "InfectionSummaryAbsoluteReward",
    "SmoothStageChangesReward",
    "ElderlyHospitalizedReward",
    "AverageStageReward",
    "PoliticalReward",
]
...
def _register_reward(
    type: RewardFunctionType, reward_fun: Type[RewardFunction]
) -> None:
...
class SumReward(RewardFunction):
        ...
        **kwargs: Any,
        ...
        if weights is not None:
            assert len(weights) == len(
                reward_fns
            ), "There must be one weight for each reward function."
        else:
    ...
    def calculate_reward(
        ...
        rewards = np.array(
            [rf.calculate_reward(prev_obs, action, obs) for rf in self._reward_fns]
        )
        rewards_mapping = np.array([type(rf).__name__ for rf in self._reward_fns])
        return float(np.sum(rewards * self._weights)), dict(
            zip(rewards_mapping, rewards)
        )
...
class InfectionSummaryIncreaseReward(RewardFunction):
    ...
    def calculate_reward(
        ...
        if np.any(prev_summary == 0):
            return 0
        return -1 * float(
            np.clip((summary - prev_summary) / prev_summary, 0, np.inf).mean()
        )
...
class InfectionSummaryAboveThresholdReward(RewardFunction):
    ...
    def calculate_reward(
        ...
        return float(
            ...
            * max(
                np.mean(
                    obs.global_infection_summary[..., self._index] - self._threshold
                )
                / self._threshold,
                0,
            )
...
class UnlockedBusinessLocationsReward(RewardFunction):
    ...
    def __init__(
        self, obs_indices: Optional[Sequence[int]] = None, *args: Any, **kwargs: Any
    ):
        """
    ...
    def calculate_reward(
        ...
        if obs.unlocked_non_essential_business_locations is None:
            ...
            unlocked_locations = (
                obs.unlocked_non_essential_business_locations
                if self._obs_indices is None
                else obs.unlocked_non_essential_business_locations[
                    ..., self._obs_indices
                ]
            )
...
class PoliticalReward(RewardFunction):
    ...
    def calculate_reward(
        ...
        infection_rate = (
            np.mean(prev_obs.global_infection_summary[-1, -1, self._infected_idx])
...
_register_reward(
    RewardFunctionType.INFECTION_SUMMARY_INCREASE, InfectionSummaryIncreaseReward
)
_register_reward(
...
_register_reward(
    RewardFunctionType.INFECTION_SUMMARY_ABSOLUTE, InfectionSummaryAbsoluteReward
)
_register_reward(
    RewardFunctionType.UNLOCKED_BUSINESS_LOCATIONS, UnlockedBusinessLocationsReward
)
