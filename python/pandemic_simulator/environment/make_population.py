...
def get_us_age_distribution(num_persons: int) -> List[int]:
    ...
    for i, age in enumerate(age_group):
        if age < 60:
            ...
            age_p[i] = (
                1 + (age - 60) * (0.05 - 1) / (100 - 60)
            ) * globals.numpy_rng.normal(1, 0.05)
    age_p /= np.sum(age_p)
    ages = [
        int(globals.numpy_rng.choice(np.arange(1, 101), p=age_p))
...
def infection_risk(age: int) -> Risk:
    return cast(
        Risk,
        globals.numpy_rng.choice(
            [Risk.LOW, Risk.HIGH], p=[1 - age / age_group.stop, age / age_group.stop]
        ),
    )
...
def make_population(sim_config: PandemicSimConfig) -> List[Person]:
    ...
    clustered_nursing_ages = cluster_into_random_sized_groups(
        retirees_in_nursing_ages, 1, 2, numpy_rng
    )
    retiree_homes_ages = [
        (unassigned_homes[_i], _a)
        for _i, _g in enumerate(clustered_nursing_ages)
        for _a in _g
    ]
    nursing_homes = unassigned_homes[: len(clustered_nursing_ages)]
    unassigned_homes = unassigned_homes[
        len(nursing_homes) :
    ]  # remove assigned nursing homes
    ...
    for home, age in retiree_homes_ages:
        persons.append(
            Retired(
                person_id=PersonID(f"retired_{str(uuid4())}", age),
                home=home,
    ...
    assert len(unassigned_homes) >= len(
        clustered_minor_ages
    ), "not enough homes to assign all people"
    minor_homes_ages = [
        (unassigned_homes[_i], _a)
        for _i, _g in enumerate(clustered_minor_ages)
        for _a in _g
    ]
    minor_homes = unassigned_homes[: len(clustered_minor_ages)]
    unassigned_homes = unassigned_homes[
        len(minor_homes) :
    ]  # remove assigned minor homes
    # create all minor
    for home, age in minor_homes_ages:
        persons.append(
            Minor(
                person_id=PersonID(f"minor_{str(uuid4())}", age),
                home=home,
    ...
    required_num_adults = (
        len(minor_homes) + len(unassigned_homes) - len(unassigned_retiree_ages)
    )
    ...
    for home, age in adult_homes_ages:
        ...
        assert (
            work_package
        ), "Not enough available jobs, increase the capacity of certain businesses"
        persons.append(
            Worker(
                person_id=PersonID(f"worker_{str(uuid4())}", age),
                home=home,
