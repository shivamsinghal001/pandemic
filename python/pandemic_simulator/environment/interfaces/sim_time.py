...
class SimTime:
    ...
    def now(self, frmt: str = "ydwh") -> List[int]:
        ...
        for _t in frmt:
            assert _t in _map, (
                f"Unrecognized frmt string given {frmt}. frmt must be a "
...
class SimTimeInterval:
    ...
    def __post_init__(self) -> None:
        ...
        assert (
            self.hour + self.day + self.year > 0
        ), "Provide a non-zero value at least for either of hour/day/year"
    ...
    def trigger_at_interval(self, sim_time: SimTime) -> bool:
        ...
        return (
            (sim_hr - self._offset_hr) % self._trigger_hr == 0
            if sim_hr >= self._offset_hr
            else False
        )
