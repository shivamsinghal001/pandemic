...
class BasePerson(Person):
    ...
    def __init__(
        ...
        self._init_state = init_state or PersonState(
            infection_state=None,
            infection_state_delta=None,
    ...
    def _sync(self, sim_time: SimTime) -> None:
        ...
        if (
            self._state.current_location not in self.assigned_locations
            and self._registry.is_location_open_for_visitors(
                self._state.current_location, sim_time
            )
        ):
            self._go_home = True
    ...
    def _set_is_hospitalized(self, value: bool) -> None:
        ...
        self._state.infection_state = dataclasses.replace(
            self._state.infection_state, **inf_state_dict
        )
    ...
    def step(
        ...
        is_hospitalized = (
            self._state.infection_state is not None
            and self._state.infection_state.is_hospitalized
        )
        is_hospitalized = is_hospitalized or (
            self._state.infection_state_delta is not None
            and self._state.infection_state_delta.is_hospitalized
        )
        if test_result == PandemicTestResult.DEAD:
            ...
            if len(self._cemetery_ids) > 0 and curr_loc not in self._cemetery_ids:
                self.enter_location(
                    self._cemetery_ids[
                        self._numpy_rng.randint(0, len(self._cemetery_ids))
                    ]
                )
        ...
        comply_to_regulation = (
            self._numpy_rng.uniform() < self._regulation_compliance_prob
        )
        if (
            not self._registry.get_person_quarantined_state(self._id)
            and comply_to_regulation
            and
            # if you are tested sick and have to stay home
            (
                self._state.test_result
                in {PandemicTestResult.POSITIVE, PandemicTestResult.CRITICAL}
                and not self.at_home
                and self._state.sick_at_home
            )
            or
            # quarantine
            (self._state.quarantine and not self.at_home)
            or
            # quarantine if household positive
            (
                self._state.quarantine_if_household_quarantined
                and not self.at_home
                and self._household_quarantined()
            )
            or
            # quarantine if contact positive
            (
                contact_tracer is not None
                and self._state.quarantine_if_contact_positive
                and not self.at_home
                and self._contact_positive(
                    list(contact_tracer.get_contacts(self.id).keys())
                )
            )
        ):
            self.enter_location(self.home)
    ...
    def receive_regulation(self, regulation: PandemicRegulation) -> None:
        ...
        self._state.quarantine_if_contact_positive = (
            regulation.quarantine_if_contact_positive
        )
        self._state.quarantine_if_household_quarantined = (
            regulation.quarantine_if_household_quarantined
        )
        self._state.sick_at_home = regulation.stay_home_if_sick
        self._state.avoid_gathering_size = regulation.risk_to_avoid_gathering_size[
            self._state.risk
        ]
        self._state.avoid_location_types = (
            regulation.risk_to_avoid_location_types[self._state.risk]
            if regulation.risk_to_avoid_location_types is not None
            else []
        )

        self._state.infection_spread_multiplier *= (
            0.8 if regulation.practice_good_hygiene else 1.0
        )
        self._state.infection_spread_multiplier *= (
            0.6 if regulation.wear_facial_coverings else 1.0
        )
        self._state.infection_spread_multiplier = (
            1
            - (1 - self._state.infection_spread_multiplier)
            * self._regulation_compliance_prob
        )
        ...
        self._state.infection_spread_multiplier_delta = (
            1
            - (1 - self._state.infection_spread_multiplier_delta)
            * self._regulation_compliance_prob
        )

    def _contact_positive(self, contacts: Sequence[PersonID]) -> bool:
        for contact in contacts:
            if self._registry.get_person_test_result(contact) in {
    ...
    def get_social_gathering_location(self) -> Optional[LocationID]:
        ...
        if num_events != 0:
            for i in self._numpy_rng.permutation(num_events):
                if (
                    ags == -1
                    or not comply_to_regulation
                    or (
                        comply_to_regulation
                        and len(self._registry.get_persons_in_location(loc_ids[i]))
                        < ags
                    )
                ):
    ...
    def reset(self) -> None:
        ...
        self._registry.clear_quarantined(self._id)
        self._registry.register_person_entry_in_location(
            self.id, self._state.current_location
        )
