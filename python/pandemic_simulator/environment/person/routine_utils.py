    ...
    for rws in routines_with_status:
        if rws.started and not rws.done:
            if (
                ...
                (person.state.current_location != rws.end_loc_selected)
                or
                # duration elapsed
    ...
    for rws in routines_with_status:
        ...
        if rws.due:
            if (
                ...
                (
                    routine.start_loc is None
                    or routine.start_loc == person.state.current_location
                )
                and
                ...
                if (len(routine.explorable_end_locs) > 0) and (
                    numpy_rng.uniform() < routine.explore_probability
                ):
                    end_loc = routine.explorable_end_locs[
                        numpy_rng.randint(0, len(routine.explorable_end_locs))
                    ]
...
def _get_locations_from_type(
    location_type: Type,
) -> Tuple[LocationID, Sequence[LocationID]]:
    assert (
        globals.registry
    ...
    end_loc = explorable_end_locs[
        globals.numpy_rng.randint(0, len(explorable_end_locs))
    ]
...
def triggered_routine(
    ...
    return PersonRoutine(
        start_loc=start_loc,
        end_loc=end_loc,
        start_trigger=SimTimeRoutineTrigger(
            day=interval_in_days,
...
def mid_day_during_week_routine(
    ...
    return PersonRoutine(
        ...
        valid_time=SimTimeTuple(
            hours=tuple(range(11, 14)), week_days=tuple(range(0, 5))
        ),
        explorable_end_locs=explorable_end_locs,
        explore_probability=explore_probability,
    )
