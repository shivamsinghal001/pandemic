...
__all__ = [
    ...
    "plot_critical_summary",
    "plot_global_infection_summary",
    "plot_multi_params_summary",
    "plot_deaths_per_day_summary",
    "get_stage_strategy",
    "plot_annotate_stages",
    "inf_colors",
]
...
def plot_global_infection_summary(
    ...
    gis = (
        exp_result.obs_trajectories.global_infection_summary
    ...
    title = (
        "Global Infection Summary" if not testing_summary else "Global Testing Summary"
    )
...
def plot_critical_summary(
    ...
    ax.plot(
        np.arange(critical.shape[0]),
        np.ones(critical.shape[0]) * max_hospital_capacity,
        "y",
        label="max_cap",
    )
...
def plot_deaths_per_day_summary(
    exp_result: ExperimentResult,
    show_variance: bool = True,
    annotate_stages: bool = False,
    ax: Optional[Axes] = None,
    ...
    if num_seeds > 1:
        ...
        if show_variance:
            ...
            ax.fill_between(
                np.arange(len(deaths_per_day_mean)),
...
def _get_ylims(
    avgs: List[Sequence[float]],
    stds: List[Sequence[float]],
    ylim_max_offset: float = 0.0,
) -> Tuple[float, float]:
    ylim_min = np.inf
    ylim_max = 0
...
def _get_yerr(
    avg: Sequence[float], std: Sequence[float]
) -> Union[Sequence[float], Tuple[Sequence[float], Sequence[float]]]:
    if np.min(avg) > 0:
...
def _get_t_scores(avg: List[float], std: List[float]) -> List[float]:
    ...
    for i, (m1, s1) in enumerate(zip(avg, std)):
        for m2, s2 in zip(avg[i + 1 :], std[i + 1 :]):
            t_scores.append(
                np.sqrt(len(avg)) * np.abs(m2 - m1) / np.sqrt(s1**2 + s2**2)
            )
    return t_scores


def plot_multi_params_summary(
    exp_results: Sequence[ExperimentResult],
    ...
    if axs is None:
        fig, axs = (
            ...
            if not show_testing_diff_plot
            else plt.subplots(3, 2, figsize=(8, 6))
        )
        axs = list(np.ravel(axs))

    ylabel_size = 8
    assert len(exp_results) == len(
        param_labels
    ), f"{len(exp_results)}, {len(param_labels)}"
    ...
    for i, (exp_result, max_hospitals_capacity) in enumerate(
        zip(exp_results, max_hospitals_capacities)
    ):
        ...
        deaths_peak_times = (
            deaths_per_day.argmax(axis=0) + 1
        )  # add 1 to account for diff on first day
        ...
        infection_peak_times_avg.append(
            (infection_peak_times - first_infection_day).mean()
        )
        infection_peak_times_std.append(
            (infection_peak_times - first_infection_day).std()
        )

        critical_peak_times_avg.append(
            (critical_peak_times - first_critical_day).mean()
        )
        ...
        critical = exp_result.obs_trajectories.global_infection_summary[
            ..., critical_index
        ]
        critical_above_cap = np.clip(
            (critical - max_hospitals_capacity) / max_hospitals_capacity, 0, np.inf
        ).sum(axis=0)
        ...
        pandemic_duration = np.max(
            (last_infection_day, last_critical_day), axis=0
    ...
    axs_2d[plot_i_2d].bar(
        index,
        infection_peaks_avg,
        bar_width,
        alpha=opacity,
        color="c",
        yerr=_get_yerr(infection_peaks_avg, infection_peaks_std),
    ...
    axs_2d[plot_i_2d].set_xticklabels(
        param_labels, rotation=xticks_rotation, fontsize=xticks_fontsize
    )
    ...
    axs_2d[plot_i_2d].set_ylim(
        _get_ylims([critical_over_capacity_avg], [critical_over_capacity_std])
    )
    ...
    axs_2d[plot_i_2d].bar(
        ...
        alpha=opacity,
        color="k",
        yerr=_get_yerr(total_deaths_avg, total_deaths_std),
        error_kw={"elinewidth": 1.0},
    )
    axs_2d[plot_i_2d].set_xlabel(xlabel)
    axs_2d[plot_i_2d].set_ylabel("persons / population size", fontsize=ylabel_size)
    ...
    if show_time_to_peak:
        ...
        axs_2d[plot_i_2d].legend(
            ["Infection", "Critical", "Death"], loc="upper right", ncol=3, fontsize=6
        )
        axs_2d[plot_i_2d].set_ylim(
            _get_ylims(
                [
    ...
    if show_pandemic_duration:
        ...
        axs_2d[plot_i_2d].set_ylim(
            _get_ylims([pandemic_duration_avg], [pandemic_duration_std])
        )
    ...
    if show_testing_diff_plot:
        ...
        axs_2d[plot_i_2d].bar(
            ...
            alpha=opacity,
            color="g",
            yerr=_get_yerr(gts_diff_avg, gts_diff_std),
            error_kw={"elinewidth": 1.0},
        )
        axs_2d[plot_i_2d].set_xlabel(xlabel)
