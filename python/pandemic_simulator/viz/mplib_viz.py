...
class PlotType:
    ...
    def plot_order() -> List[str]:
        return [
            PlotType.global_infection_summary,
            PlotType.global_testing_summary,
            PlotType.critical_summary,
            PlotType.stages,
            PlotType.location_assignee_visits,
            PlotType.location_visitor_visits,
            PlotType.infection_source,
            PlotType.cumulative_reward,
            PlotType.cumulative_true_reward,
        ]
...
class BaseMatplotLibViz(PandemicViz):
    ...
    def __init__(self, num_persons: int, max_hospital_capacity: Optional[int] = None):
        ...
        self._max_hospital_capacity = max_hospital_capacity or min(
            1, int(0.01 * num_persons)
        )
    ...
    @classmethod
    def from_config(
        cls: Type["BaseMatplotLibViz"], sim_config: PandemicSimConfig
    ) -> "BaseMatplotLibViz":
        return cls(
    ...
    def record_obs(self, obs: PandemicObservation) -> None:
        if len(self._gis_legend) == 0:
            ...
            self._critical_index = self._gis_legend.index(
                InfectionSummary.CRITICAL.value
            )
    ...
    def plot_critical_summary(self, ax: Optional[Axes] = None, **kwargs: Any) -> None:
        ...
        ax.plot(
            np.arange(gis.shape[0]),
            np.ones(gis.shape[0]) * self._max_hospital_capacity,
            "y",
        )
    ...
    def plot(
        self,
        name=datetime.now().strftime("%m-%d-%Y-%H_%M_%S"),
        plots_to_show: Optional[Sequence[str]] = None,
        *args: Any,
        **kwargs: Any,
    ) -> None:
        if plots_to_show:
            fn_names = [
                nm for nm in plots_to_show if ismethod(getattr(self, "plot_" + nm))
            ]
        else:
            fn_names = [
                nm.split("plot_")[-1]
                for nm in dir(self)
                if nm.startswith("plot_") is True
            ]
            fn_names = [
                nm
                for nm in sorted(
...
class SimViz(BaseMatplotLibViz):
    ...
    def __init__(self, num_persons: int, max_hospital_capacity: Optional[int] = None):
        ...
        """
        super().__init__(
            num_persons=num_persons, max_hospital_capacity=max_hospital_capacity
        )
    ...
    def record_state(self, state: PandemicSimState) -> None:
        ...
        if len(self._loc_types) == 0:
            self._loc_types = sorted(
                set(k[0] for k in state.global_location_summary.keys())
            )
            self._person_types = sorted(
                set(k[1] for k in state.global_location_summary.keys())
            )
        ...
        for i in range(len(self._loc_types)):
            for j in range(len(self._person_types)):
                ec = state.global_location_summary[
                    (self._loc_types[i], self._person_types[j])
                ].entry_count
                vc = state.global_location_summary[
                    (self._loc_types[i], self._person_types[j])
                ].visitor_count
                _av[0, i, j] = ec - vc
                _vv[0, i, j] = vc
        ...
        self._location_type_to_is = {
            k.__name__: v for k, v in state.location_type_infection_summary.items()
        }

    def plot_location_assignee_visits(
    ...
    def plot_infection_source(self, ax: Optional[Axes] = None, **kwargs: Any) -> None:
        ...
        if len(self._location_type_to_is) > 0:
            ...
            ax.bar(
                x,
                [
                    self._location_type_to_is[k] / self._num_persons
                    for k in self._loc_types
                ],
                color="r",
                alpha=0.5,
            )
...
class GymViz(BaseMatplotLibViz):
    ...
    def plot_cumulative_true_reward(
        self, ax: Optional[Axes] = None, **kwargs: Any
    ) -> None:
        ax = ax or plt.gca()
    ...
    def plot_gis(self, ax: Optional[Axes] = None, **kwargs: Any) -> None:
        ...
        for mean, std in zip(np.hsplit(gis, 5), np.hsplit(gis_std, 5)):
            ax.plot(days, mean)
            ax.fill_between(
                days, (mean - std).squeeze(1), (mean + std).squeeze(1), alpha=0.1
            )
    ...
    def plot_critical_summary(self, ax: Optional[Axes] = None, **kwargs: Any) -> None:
        ...
        ax.fill_between(
            np.arange(len(gis)),
            self._num_persons * (gis - gis_std)[:, self._critical_index],
            self._num_persons * (gis + gis_std)[:, self._critical_index],
            alpha=0.1,
        )
        ax.plot(
        ...
        ax.legend(
            [InfectionSummary.CRITICAL.value, "Max hospital capacity"], loc="upper left"
        )
        ax.set_ylim(-0.1, self._max_hospital_capacity * 3)
        ax.set_title("Critical Summary")
        ax.set_xlabel("time (days)")
